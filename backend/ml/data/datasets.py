...
def preprocess_image(image, model_type='malaria'):
    ...
    try:
        ...
        # Use image sizes that match training
        if model_type == 'skin_cancer':
            target_size = (128, 128)  # Matches training size
        else:  # malaria
            target_size = (96, 96)    # Matches training size
            
        # Get the transform pipeline with ImageNet normalization (matches training)
        transform = transforms.Compose([
